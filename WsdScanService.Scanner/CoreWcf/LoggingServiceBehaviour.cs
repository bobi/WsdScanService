...
namespace WsdScanService.Scanner.CoreWcf;

public class LoggingServiceBehaviour(ILogger<LoggingServiceBehaviour> logger) : IServiceBehavior
{
...
internal class LoggingErrorHandler<T>(ILogger<T> logger) : IErrorHandler
{
    ...
    public void ProvideFault(Exception error, MessageVersion version, ref Message fault)
    {
        if (Logger.IsEnabled(LogLevel.Error))
        {
            Logger.LogError("An error occurred while processing the request. {}, {}, {}", version, fault, error);
        }
    }
